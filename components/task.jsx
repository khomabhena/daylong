  ...
  return (
    <div className='px-4 pb-8'>
        <div className=' h-80 overflow-auto mb-4'>
            ...
            <div className=' mt-4'>
                <h2 onClick={handleMain} className=' cursor-pointer text-lg text-slate-400 font-semibold'>{mainData.name}</h2>
                ...
                <ol className=' mt-2 text-slate-400'>
                    {
                        data.length >= 1 && data?.map(({_id, name, started, finished}) => (
                            <li key={_id} className=' mx-4 bg-slate-900/50 px-4 py-2 mb-2 rounded-md'>
        ...
        <form onSubmit={handleCreateTask} className=' flex flex-col'>
            <input onChange={(e) => setName(e.target.value)} ref={refName} type="text" placeholder='create task' className=' border border-emerald-600 bg-slate-950/50 rounded-md shadow-sm shadow-emerald-600 text-slate-300' />
